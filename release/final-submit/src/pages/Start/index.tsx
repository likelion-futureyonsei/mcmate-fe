...
import {clearLikedItems} from "@/hooks/useLikedItem";
...
export function Start() {
  ...
  useEffect(() => {
    ...
    const characterSelectTimer = window.setTimeout(
      () => navigate("/dog-detail", {state: {hideBottomNav: true}}),
      5000,
    );

    return () => window.clearTimeout(characterSelectTimer);
  }, [navigate, screen]);

  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    window.localStorage.removeItem(PRODUCT_REGISTERED_STORAGE_KEY);
    window.localStorage.removeItem(PRODUCT_WRITTEN_STORAGE_KEY);
    clearLikedItems();
    setScreen("loading");
  };
