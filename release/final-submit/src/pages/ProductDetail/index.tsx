...
const PRODUCT_DETAIL_BEFORE_WRITTEN_IMAGE = "/product-add-complete-screen.svg?v=20260821-product-detail";
const PRODUCT_DETAIL_WRITTEN_IMAGE = "/product-detail-written-screen.svg?v=20260821-written-detail";

type ProductDetailLocationState = {
  didRegisterProduct?: boolean;
  returnTo?: string;
};
...
export function ProductDetail() {
  ...
  const locationState = state as ProductDetailLocationState | null;
  const returnTo = locationState?.returnTo;

  useEffect(() => {
    if (locationState?.didRegisterProduct) {
      window.localStorage.setItem(PRODUCT_REGISTERED_STORAGE_KEY, "true");
    }
  }, [locationState?.didRegisterProduct]);
  ...
  return (
    <section className={styles.page} aria-label="product detail page">
      <img
        ...
        src={hasWrittenProduct ? PRODUCT_DETAIL_WRITTEN_IMAGE : PRODUCT_DETAIL_BEFORE_WRITTEN_IMAGE}
        data-written={hasWrittenProduct}
        alt=""
        aria-hidden="true"
      />
      ...
      {hasWrittenProduct && (
        <Link className={styles.firstGalleryHotspot} to="/product-photo" aria-label="첫 번째 에피소드 사진 보기" />
      )}
    </section>
  );
}
